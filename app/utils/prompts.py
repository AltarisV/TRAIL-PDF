PROMPTS = {
    ...
    'turkish': (
        "Erişilebilir içerik oluşturarak materyallerin erişilebilir formatlara dönüştürülmesine yardımcı olmakla görevlendirildiniz. "
        "Size bir üniversite ders slaytı veriyorum. Slaytın tamamı için alternatif metin olarak kullanılabilecek bir açıklama oluşturun. "
        "Tamamen görme engelli bir öğrenci çizimi anlayabilmelidir. Açıklamanızda dekoratif unsurlar göz ardı edilmelidir. "
        "Erişilebilirlik son derece önemlidir. Görseldeki çağrışımları tanıyorsanız bunları açıklayın. "
        "Görsel üzerinde normal metin görünüyorsa, metni değiştirmeden, ancak İngilizce olarak alternatif metne dahil ederek yazın. "
        "Orijinal metin İngilizce değilse, yanıtınız yalnızca çevrilmiş İngilizce metni içermelidir."
        "Cevabınız bir liste içeriyorsa, liste html formatında olmalıdır."
        "\n\n"
        "Matematiksel formüller mevcutsa, bu formülün kesin bir açıklamasını erişilebilir bir metinde, sanki yüksek sesle okunuyormuş gibi yazın. "
        "'(' yerine 'açık parantez' ve ')' yerine 'kapalı parantez' yazın. Bir kesir varsa, pay "
        "ve paydayı bu şekilde adlandırmalısınız ('pay bir bölü payda iki eşittir 0,5'). Erişilebilir bir formül örneği şöyledir: "
        "\"Dizin 1'den n'ye kadar toplam x'e eşittir noktalı virgül mu virgül beta eksi 1'in kuvvetine yükseltilmiş son üs eşittir payın karekökü beta "
        "bölü payda 2 pi kesir sonuç kök son exp açık parantez eksi bir yarım kesir beta açık parantez x eksi mu kapalı parantez "
        "karesi kapalı parantez.\""
        "\n\n"
        "Resim bir tablo içeriyorsa, bu tabloyu HTML koduna dönüştürmek sizin görevinizdir, böylece görme engelli bir öğrencinin "
        "ekran okuyucularıyla erişebileceği bir HTML tablosu haline gelir. Yanıtınız, tablo için <thead>, <tbody> ve <th scope> etiketleriyle birlikte HTML kodunu ve tablo için bir "
        "<caption> içermelidir. Yanıt markdown içermemelidir."
        "\n\n"
        "Resim kod içeriyorsa, bu kodu bir HTML <code> etiketi içinde yeniden üretmek sizin görevinizdir. Resim kod içeriyorsa işte bir örnek:\n"
        "<code>def hello_world():\n print('hello world!')</code>"
        "\n\n"
        "Genel olarak, stil, tasarım veya kurumun logosu hakkında hiçbir şey yazmayın. Bu son derece önemlidir. "
        "Mesajınız 'Sayfa {sayfa_numarası}, {sizin tarafınızdan oluşturulan bir-iki kelimelik bir başlık}:' ile başlamalıdır. Başlık İngilizce olmalı ve sizin tarafınızdan seçilmelidir, . "
        "Yanıtınızda slaytın başlığını tekrarlamayın. Yanıtınıza ‘Alternatif metin:’ ile BAŞLAMAYIN."
    ),
    'french': (
        "Vous êtes chargé(e) d'aider à convertir les documents en formats accessibles en créant un contenu accessible. "
        "Je vous fournis une diapositive de cours universitaire. Générez une description qui peut être utilisée comme texte alternatif pour l'ensemble de la diapositive. "
        "Un étudiant atteint de cécité totale devrait être en mesure de comprendre l'illustration. Les éléments décoratifs doivent être ignorés dans votre description. "
        "L'accessibilité est de la plus haute importance. Si vous reconnaissez des associations sur l'image, décrivez-les. "
        "Si un texte normal est visible sur l'image, écrivez le texte sans changement, mais en anglais, en l'incluant dans le texte alternatif. "
        "Si le texte original n'est pas en anglais, votre réponse ne doit comprendre que le texte anglais traduit."
        "Si votre réponse comprend une liste, la liste doit être en html."
        "\n\n"
        "Si des formules mathématiques sont présentes, écrivez une description précise de cette formule dans un texte accessible, comme s'il s'agissait d'une lecture à haute voix. "
        "Au lieu de '(', écrivez 'parenthèses ouvertes', et au lieu de ')', écrivez 'parenthèses fermées'. Si une fraction est présente, vous devez nommer le numérateur "
        "et le dénominateur comme tels ('numérateur un divisé par dénominateur deux est égal à 0,5'). Voici un exemple de formule accessible : "
        "\"La somme de l'indice 1 à n est égale à x point-virgule mu virgule bêta élevé à la puissance de moins 1 fin exposant égale racine carrée du numérateur bêta "
        "divisé par le dénominateur 2 pi fraction résultat racine fin exp parenthèse ouverte moins une demi fraction bêta parenthèse ouverte x moins mu parenthèse fermée "
        "au carré parenthèse fermée.\""
        "\n\n"
        "Si l'image comprend un tableau, il vous incombe de convertir ce tableau en code HTML, afin qu'il devienne un tableau HTML dans lequel un étudiant aveugle puisse naviguer "
